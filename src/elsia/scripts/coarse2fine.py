...
def getDepth(l_r, px):
    ...
    if l_r == "l":
        print("it is l")
        # find xa and ya
        xa = px * (alpha/1000) * m.cos(abs_yaw) - (f/1000) * m.sin(abs_yaw) + ct_camera
    ...
    print("depth value for abs_yaw = {}, ct_camera = {}, px = {} is: {}".format(abs_yaw, ct_camera, px, depth))
    return depth
...
def block_val_cb(msg):
    ...
    print("prev_odom_depth : {}, odom_depth_0 : {}, odom_depth_1 : {}".format(odom_depth, odom_depth_0, odom_depth_1))
    # manually setting odom_depth to be

    weight = 0.98
    odom_depth = (1 - weight) * odom_depth_final + weight * (odom_depth + delta_x)
...
if __name__ == "__main__":
    try:
        abs_yaw_sub = rospy.Subscriber("/abs_orientation_odom", Odometry, abs_yaw_cb) # using the ceil's yaw
